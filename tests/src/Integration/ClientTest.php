...
use JonnyW\PhantomJs\Tests\TestCase;
...
EOF;
    ...
    {
        $this->expectException(\JonnyW\PhantomJs\Exception\SyntaxException::class);
...
EOF;
    ...
    public function testResponseContainsValidBodyIfPageIsSuccessfullyLoaded()
    {
        ...
        static::assertStringContainsString('Example Domain', $response->getContent());
    }
    ...
    public function testCanSetUserAgentInSettings()
    {
        ...
        $request->setMethod('GET');
        $request->setUrl('https://gabrielsolomon.ro/test/');
        $request->addSetting('userAgent', 'PhantomJS TEST USER AGENT');

        $client->send($request, $response);

        static::assertStringContainsString('PhantomJS TEST USER AGENT', $response->getContent());
    }
    ...
    public function testCanAddCookiesToRequest()
    {
        ...
        $request->setMethod('GET');
        $request->setUrl('https://gabrielsolomon.ro/test/');
        $request->addCookie('test_cookie', 'TESTING_COOKIES', '/', '.gabrielsolomon.ro');

        $client->send($request, $response);

        static::assertStringContainsString('test_cookie', $response->getContent());
        static::assertStringContainsString('TESTING_COOKIES', $response->getContent());
    }
    ...
    public function testCanLoadCookiesFromPersistentCookieFile()
    {
        ...
        $request->setMethod('GET');
        $request->setUrl('https://gabrielsolomon.ro/test/');
        $request->addCookie('test_cookie', 'TESTING_COOKIES', '/', '.gabrielsolomon.ro', true, false,
            ($expireAt * 1000));

        $client->send($request, $response);

        static::assertStringContainsString('test_cookie=TESTING_COOKIES; HttpOnly; expires=Sun, 15-Nov-2020 22:00:00 GMT; domain=.gabrielsolomon.ro; path=/',
    ...
    public function testCookiesPresentInResponse()
    {
        ...
        static::assertSame('.jonnyw.kiwi', $cookie['domain']);
        static::assertSame('TESTING_COOKIES', $cookie['value']);
    }
    ...
    public function testResponseContainsConsoleErrorIfAJavascriptErrorExistsOnThePage()
    {
        ...
        static::assertStringContainsString('ReferenceError: Can\'t find variable: invalid', $console[0]['message']);
    }
    ...
    public function testPostRequestSendsRequestData()
    {
        ...
        foreach ($data as $value) {
            static::assertStringContainsString($value, $content);
        }
    }
    ...
    public function testCanSetRepeatingHeaderForPDFRequest()
    {
        ...
        static::assertStringContainsString('Header', $text);
    }
    ...
    public function testCanSetRepeatingFooterForPDFRequest()
    {
        ...
        static::assertStringContainsString('Footer', $text);
    }
    ...
    public function testDelayDelaysPageRenderForSpecifiedTimeForDefaultRequest()
    {
        ...
        static::assertSame(($startTime + $delay), $endTime);
    }
    ...
    public function testLazyRequestReturnsResourcesAfterAllResourcesAreLoaded()
    {
        ...
        $request->setMethod('GET');
        $request->setUrl('https://enviragallery.com/demo/lazy-loading-demo/');
        $request->setTimeout(5000);

        $client->send($request, $response);

        static::assertStringContainsString('<div data-envira-id="225278" data-gallery-config="',
            $response->getContent());
    }
    ...
    public function testContentIsReturnedForLazyRequestIfTimeoutIsReachedBeforeResourceIsLoaded()
    {
        ...
        static::assertStringContainsString('This gallery has lazy loading enabled.', $response->getContent());
    }
...
//        $client->send($request, $response);
//
//        static::assertStringContainsString('[DEBUG]', $client->getLog());
    }
    ...
    public function testCanSetPageBackgroundColor()
    {
        ...
        static::assertStringContainsString('body style="background-color: red;"', $response->getContent());
    }
    ...
    protected function getClient()
    {
        ...
        $engine = $serviceContainer->get('engine');
        $engine->setPath(PhantomBinary::getBin());
